...
export const deleteDeviceByAdmin = async (deviceId) => {
    const { data } = await $authHost.delete(`/device/admin/${deviceId}`);
    return data;
};

export const deleteBrand = async (brandId) => {
    const { data } = await $authHost.delete(`/brand/${brandId}`);
    return data;
};

export const deleteType = async (typeId) => {
    const { data } = await $authHost.delete(`/type/${typeId}`);
    return data;
};
